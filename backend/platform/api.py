...
import os, tempfile, uuid, json, re
...
from backend.engine.world_services import MapSystem, WorldManager

_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")

def install(app, worlds, memories, store):
    bp = Blueprint('platform', __name__, url_prefix='/api/v2')
    data_root = Path(os.getenv('RPG_DATA_DIR', str(Path(__file__).resolve().parents[2] / 'data'))).resolve()
    runtime = RPGRuntime(str(data_root)); backups = BackupManager(str(data_root)); notebooks = {}; maps = MapSystem(); manager = WorldManager()
    def valid_id(value):
        value = str(value)
        if not _ID_RE.fullmatch(value): raise ValueError('Identificador inválido')
        return value
    def notebook(wid):
        wid = valid_id(str(wid).zfill(3))
        if wid not in notebooks: notebooks[wid] = NotebookWorkspace(wid, data_root / 'notebooks' / wid)
        return notebooks[wid]
    def world_or_404(wid): return worlds.get(str(wid).zfill(3))
    @bp.get('/status')
    def status(): return jsonify({'ok': True, 'versao': '0.9.2', 'arquitetura': 'estado->simulacao->memoria->rag->llm', 'entidades': len(runtime.entities), 'notebooks': len(notebooks), 'guard': True, 'agente': True, 'autonomia': True, 'backup': True})
    ...
    def state(world_id):
        ...
        runtime.sync_entities_from_world(w); return jsonify({'world': manager.normalize(w), 'entities': [asdict(e) for e in runtime.entities.values() if e.world_id == wid]})
    ...
    def advance(world_id):
        ...
        try:
            r = runtime.simulate(w, float(b.get('hours', 1))); errors = runtime.audit.validate(w)
    ...
    def snapshot(world_id):
        wid = str(world_id).zfill(3); w = world_or_404(wid)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        manager.normalize(w); runtime.sync_entities_from_world(w); runtime.persist_entities_to_world(w); worlds.save(w); b = request.get_json(silent=True) or {}
        return jsonify({'snapshot': runtime.snapshots.save(wid, w, str(b.get('label') or 'manual'))}), 201
    ...
    @bp.get('/worlds/<world_id>/export')
    def export_world(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        payload = json.dumps(manager.exportable(w), ensure_ascii=False, indent=2); return Response(payload, mimetype='application/json', headers={'Content-Disposition': f'attachment; filename=mundo-{str(world_id).zfill(3)}.json'})
    ...
    def entity(world_id):
        ...
        try:
            valid_id(item.id); runtime.sync_entities_from_world(w); runtime.register_entity(item); runtime.persist_entities_to_world(w); errors = runtime.audit.validate(w)
            if errors: return jsonify({'error': 'Estado inválido', 'erros': errors}), 409
            worlds.save(w)
        except (ValidationError, ValueError) as e: return jsonify({'error': str(e)}), 400
    ...
    def production(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        b = request.get_json(silent=True) or {}; stock = w.setdefault('economia', {}).setdefault('estoques', {}); r = runtime.production.produce(stock, b.get('recipe') or {}, int(b.get('workers', 1)), float(b.get('periods', 1)))
        if not r.get('ok'): return jsonify(r), 409
        worlds.save(w); return jsonify(r), 200
    @bp.post('/worlds/<world_id>/economy/tick')
    def economy_tick(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        b = request.get_json(silent=True) or {}; w['economia'] = runtime.economy_system.tick(w.setdefault('economia', {}), float(b.get('periods', 1))); worlds.save(w); return jsonify(w['economia'])
    ...
    @bp.post('/worlds/<world_id>/map/location')
    def map_location(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        b = request.get_json(silent=True) or {}; item = maps.add_location(w, valid_id(str(b.get('id') or uuid.uuid4().hex[:8])), str(b.get('nome') or 'Local'), float(b.get('x', 0)), float(b.get('y', 0)), str(b.get('tipo') or 'local')); worlds.save(w); return jsonify({'local': item}), 201
    @bp.get('/worlds/<world_id>/map/distance')
    def map_distance(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        try: return jsonify({'distancia': maps.distance(w, valid_id(str(request.args.get('a'))), valid_id(str(request.args.get('b'))))})
        except (KeyError, ValueError) as e: return jsonify({'error': str(e)}), 404
    ...
    def memory_list(world_id): return jsonify({'memorias': memories.list(str(world_id).zfill(3))})
    @bp.post('/worlds/<world_id>/memory')
    def memory_create(world_id):
        b = request.get_json(silent=True) or {}
        try:
            item = memories.add(str(world_id).zfill(3), str(b.get('conteudo') or b.get('content') or ''), str(b.get('tipo') or 'fato'), float(b.get('importancia', 0.5)), b.get('tags') or [], 'usuario'); return jsonify({'memoria': item}), 201
    ...
    def memory_update(world_id, memory_id):
        b = request.get_json(silent=True) or {}
        try:
            item = memories.update(str(world_id).zfill(3), valid_id(memory_id), **b)
    ...
    def memory_delete(world_id, memory_id):
        try: ok = memories.delete(str(world_id).zfill(3), valid_id(memory_id))
        except ValueError as e: return jsonify({'error': str(e)}), 400
        if not ok: return jsonify({'error': 'Memória não encontrada'}), 404
    ...
    def rag_delete_source(world_id, source_id):
        try: ok = notebook(world_id).delete_source(valid_id(source_id))
        except ValueError as e: return jsonify({'error': str(e)}), 400
        if not ok: return jsonify({'error': 'Fonte não encontrada'}), 404
        return jsonify({'ok': True})
    @bp.post('/worlds/<world_id>/rag/sources/<source_id>/reindex')
    def rag_reindex_source(world_id, source_id):
        try: return jsonify({'source': asdict(notebook(world_id).reindex_source(valid_id(source_id)))})
    ...
    def rag_upload(world_id):
        ...
        try:
            if suffix in {'.pdf', '.docx'}:
                ...
                try:
                    nb = notebook(world_id); d = nb.ingestor.from_file(path, {'filename': up.filename}); nb.documents[d.source_id] = d; nb.index.upsert(nb.ingestor.chunk(d)); nb._save_sources()
        ...
        except (ValueError, OSError) as e: return jsonify({'error': str(e)}), 422
