...
def install(app, worlds, memories, store):
    ...
    backups = BackupManager(str(data_root))
    notebooks = {}
    maps = MapSystem()
    ...
    def world_or_404(wid):
        return worlds.get(str(wid).zfill(3))

    @bp.get('/status')
    def status():
        return jsonify({'ok': True, 'versao': '0.9.1', 'arquitetura': 'estado->simulacao->memoria->rag->llm', 'entidades': len(runtime.entities), 'notebooks': len(notebooks), 'guard': True, 'agente': True, 'autonomia': True, 'backup': True})
    ...
    def state(world_id):
        ...
        runtime.sync_entities_from_world(w)
        return jsonify({'world': manager.normalize(w), 'entities': [asdict(e) for e in runtime.entities.values() if e.world_id == wid]})
    ...
    def snapshot(world_id):
        wid = str(world_id).zfill(3); w = world_or_404(wid)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        manager.normalize(w); runtime.sync_entities_from_world(w); runtime.persist_entities_to_world(w); worlds.save(w)
        b = request.get_json(silent=True) or {}
    ...
    def export_world(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        payload = json.dumps(manager.exportable(w), ensure_ascii=False, indent=2)
        return Response(payload, mimetype='application/json', headers={'Content-Disposition': f'attachment; filename=mundo-{str(world_id).zfill(3)}.json'})
    ...
    def entity(world_id):
        ...
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        b = request.get_json(silent=True) or {}
        item = Entity(str(b.get('id') or uuid.uuid4().hex[:12]), str(b.get('name') or 'Sem nome'), str(b.get('kind') or 'npc'), wid, b.get('attributes') or {}, b.get('needs') or {}, b.get('goals') or [], b.get('relations') or {}, b.get('memory_ids') or [])
        try:
            runtime.sync_entities_from_world(w); runtime.register_entity(item); runtime.persist_entities_to_world(w); errors = runtime.audit.validate(w)
    ...
    def autonomy(world_id):
        ...
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        runtime.sync_entities_from_world(w)
        return jsonify({'decisoes': runtime.autonomy.plan([asdict(e) for e in runtime.entities.values() if e.world_id == wid])})
    ...
    def production(world_id):
        ...
        r = runtime.production.produce(stock, b.get('recipe') or {}, int(b.get('workers', 1)), float(b.get('periods', 1)))
    ...
    def map_location(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        b = request.get_json(silent=True) or {}; item = maps.add_location(w, str(b.get('id') or uuid.uuid4().hex[:8]), str(b.get('nome') or 'Local'), float(b.get('x', 0)), float(b.get('y', 0)), str(b.get('tipo') or 'local')); worlds.save(w); return jsonify({'local': item}), 201

    @bp.get('/worlds/<world_id>/map/distance')
    def map_distance(world_id):
        w = world_or_404(world_id)
        if not w: return jsonify({'error': 'Mundo não encontrado'}), 404
        try: return jsonify({'distancia': maps.distance(w, str(request.args.get('a')), str(request.args.get('b')))})
    ...
    def rag_upload(world_id):
        ...
        try:
            if suffix in {'.pdf', '.docx'}:
                ...
                d = notebook(world_id).add_text(up.filename, raw.decode('utf-8'), up.mimetype or 'text/plain', {'filename': up.filename})
