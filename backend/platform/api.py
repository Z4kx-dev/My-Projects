...
def install(app, worlds, memories, store):
    bp=Blueprint('platform',__name__,url_prefix='/api/v2'); data_root=Path(os.getenv('RPG_DATA_DIR','data')); runtime=RPGRuntime(str(data_root)); backups=BackupManager(str(data_root)); notebooks={}; maps=MapSystem(); manager=WorldManager()
    def notebook(wid):
        wid=str(wid).zfill(3)
    ...
    def advance(world_id):
        ...
        try:r=runtime.simulate(w,float(b.get('hours',1))); worlds.save(w); return jsonify(r)
    ...
    def snapshot(world_id):
        wid=str(world_id).zfill(3); w=world_or_404(wid)
        if not w:return jsonify({'error':'Mundo não encontrado'}),404
        manager.normalize(w); runtime.sync_entities_from_world(w); runtime.persist_entities_to_world(w); worlds.save(w); b=request.get_json(silent=True) or {}; return jsonify({'snapshot':runtime.snapshots.save(wid,w,str(b.get('label') or 'manual'))}),201
    ...
    def production(world_id):
        w=world_or_404(world_id)
        if not w:return jsonify({'error':'Mundo não encontrado'}),404
        b=request.get_json(silent=True) or {}; stock=w.setdefault('economia',{}).setdefault('estoques',{}); r=runtime.production.produce(stock,b.get('recipe') or {},int(b.get('workers',1)),float(b.get('periods',1))); worlds.save(w); return jsonify(r),200 if r.get('ok') else 409
    ...
    def rag_upload(world_id):
        ...
        try:
            if suffix in {'.pdf','.docx'}:
                ...
                try:d=notebook(world_id).ingestor.from_file(path,{'filename':up.filename});notebook(world_id).documents[d.source_id]=d;notebook(world_id).index.upsert(notebook(world_id).ingestor.chunk(d));notebook(world_id)._save_sources()
