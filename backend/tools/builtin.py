...
def register_builtin(registry: ToolRegistry, world_getter, world_saver, memories: MemoryStore, web: WebClient | None = None) -> None:
    web = web or WebClient()

    def world_state(world_id: str) -> dict[str, Any]:
        world = world_getter(world_id)
        return {'id': world.get('id'), 'nome': world.get('nome'), 'tipo': world.get('tipo'), 'tempo': world.get('tempo', {}), 'entidades': world.get('entidades', []), 'populacao': world.get('populacao', {}), 'economia': world.get('economia', {}), 'clima': world.get('clima', {})}
    ...
    def advance_time(world_id: str, seconds: int, reason: str = 'ação') -> dict[str, Any]:
        ...
        world = world_getter(world_id)
        result = SimulationEngine(world).advance(int(seconds), reason)
        world_saver(result)
