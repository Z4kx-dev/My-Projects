...
class ContextBuilder:
    def __init__(self, worlds: WorldRepository, chats: ChatRepository, memories: MemoryStore, max_chars: int = 24000):
        self.worlds, self.chats, self.memories = worlds, chats, memories
        self.max_chars = max(8000, max_chars)
    ...
    def build(self, world_id: str, chat_id: str, query: str) -> str:
        ...
        memories = self.memories.search(world_id, query, 6)
        messages = chat.get("mensagens", [])
        recent = messages[-10:-1] if messages else []
